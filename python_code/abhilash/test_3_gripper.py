...
for i in tqdm(range(tf), desc="Simulating", unit="step"):
    ...
    if not grasp_started:
        ...
        dq_pick = dq_from_rt(q_id, p_pick)

        grip.set_pose(dq=dq_pick)
    ...
    if t < t_fall + t_lift + t_spin:
        ...
        # # keep origin fixed at lifted position
        dq = dq_from_rt(q_spin, p_lift)
...
cloth.makeMovie(
    ...
    axis_points=None,
    world_frame=True,
    gripper_origin_history=grip.origin_history,
    gripper_R_history=grip.R_history,
    gripper_visible_history=grip.visible_history
)
